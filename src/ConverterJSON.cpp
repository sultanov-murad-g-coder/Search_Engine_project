#include "..\include\ConverterJSON.h"
 ...
 vector<string> ConverterJSON::GetTextDocuments()
 {
                ...
                if (iterator_json!=json_file.end())
                {
                    for(auto &path_file:json_file["files"])
                    {
                        try
                        {
                            ...
                            else { throw runtime_error("Error opening source_file " + (string)path_file); }
                        }

                        catch(const runtime_error &ex) { cout<<ex.what()<<"\n"; continue; }

                        catch(...) { cout << "unknown error in method GetTextDocuments 0"; continue; }
                    }

                    json_file_config.close();
                }

                else { throw runtime_error("config file is empty"); }
            }

            catch(const runtime_error &ex) { cout<<ex.what(); }

            catch(...) { cout<< "unknown error in method GetTextDocuments 1"; }
        }
    ...
    catch (...) { cout<<"unknown error in method GetTextDocuments 2"<<"\n"; }
 ...
 int ConverterJSON::GetResponsesLimit()
 {
    ...
    catch(...) { cout<<"unknown error in method GetResponsesLimit"<<"\n"; }
    ...
    try
    {
        ...
        if (file_for_write.is_open())
        {
            ...
            for(int i=0; i<answers.size(); ++i)
            {
                ...
                answers[i].empty() ? empty_data=true:empty_data=false; // проверка вектора на наличие данных
        ...
        else { throw runtime_error("Error opening answers.json file"); }
    }

    catch (const runtime_error &ex) { cout<<ex.what()<<"\n"; }

    catch (...) { cout<<"unknown error in method PutAnswers"<< "\n"; }
 }
 ...
 string ConverterJSON::GetNameProject()
 {
    ...
    catch (...) { cout<<"unknown error in method GetName"; }
